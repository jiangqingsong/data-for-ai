...
| 参数 | 第1周默认值 | 第2周实验范围 | 说明 |
|------|-----------|-------------|------|
| `chunk_size` | 1000 | 500 / 1000 / 1500 | 字符数 |
| `chunk_overlap` | 200 | 100 / 200 / 300 | 重叠字符数 |
| 分块策略 | RecursiveCharacter | 对比按段落/按标题 | LangChain 内置 |

### 3.3 设计要点

- **分块策略是第一个优化点**：第1周用默认参数先跑通，第2周做对照实验找最优参数
- **元数据必须保留**：文件名、学科、页码，这些是检索结果展示"引用来源"的基础
...
- `02-data-pipeline.md` — 非结构化数据处理（Unstructured）
...
| 天 | 主题 | 具体任务 | 预估 |
|----|------|---------|------|
| Day 1 | 分块策略实验 | ① 对比 chunk_size: 500 vs 1000 vs 1500 ② 对比 overlap: 100 vs 200 vs 300 ③ 记录不同策略的效果差异 | 1-2h |
